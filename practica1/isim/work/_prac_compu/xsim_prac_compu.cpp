#include "isim/work/_prac_compu/_prac_compu.h"
...
#include "uni9000_ver.auxlib/_a_n_d2/_a_n_d2.h"
static HSim__s6* IF1(HSim__s6 *Arch,const char* label,int nGenerics, 
va_list vap)
{
    HSim__s6 *blk = new uni9000_ver_auxlibM_a_n_d2(label); 
    return blk;
}
...
#include "uni9000_ver.auxlib/_n_a_n_d2/_n_a_n_d2.h"
static HSim__s6* IF3(HSim__s6 *Arch,const char* label,int nGenerics, 
va_list vap)
{
    HSim__s6 *blk = new uni9000_ver_auxlibM_n_a_n_d2(label); 
    return blk;
}


#include "uni9000_ver.auxlib/_n_o_r2/_n_o_r2.h"
static HSim__s6* IF4(HSim__s6 *Arch,const char* label,int nGenerics, 
va_list vap)
{
    HSim__s6 *blk = new uni9000_ver_auxlibM_n_o_r2(label); 
    return blk;
}
...
#include "uni9000_ver.auxlib/_x_n_o_r2/_x_n_o_r2.h"
static HSim__s6* IF6(HSim__s6 *Arch,const char* label,int nGenerics, 
va_list vap)
{
    HSim__s6 *blk = new uni9000_ver_auxlibM_x_n_o_r2(label); 
    return blk;
}


#include "uni9000_ver.auxlib/_x_o_r2/_x_o_r2.h"
static HSim__s6* IF7(HSim__s6 *Arch,const char* label,int nGenerics, 
va_list vap)
{
    HSim__s6 *blk = new uni9000_ver_auxlibM_x_o_r2(label); 
    return blk;
}
...
public:
    ...
    HSimConfigDecl * topModuleInstantiate() {
        ...
        (*cfgvh).addVlogModule("PracCompu", (HSimInstFactoryPtr)IF0);
        ...
        (*cfgvh).addVlogModule("glbl", (HSimInstFactoryPtr)IF15);
        HSim__s5 * topvl = 0;
        topvl = new workM_prac_compu("PracCompu");
